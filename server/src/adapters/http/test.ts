...
import { assertNotSsrfTarget } from "../ssrf.js";
  ...
  if (url && (url.protocol === "http:" || url.protocol === "https:")) {
    try {
      ...
      checks.push({
        ...
        level: "error",
        message: err instanceof Error ? err.message : "URL is not allowed (SSRF protection)",
        hint: "The configured URL resolves to a private or reserved address and cannot be used.",
      });
      return {
        adapterType: ctx.adapterType,
        status: summarizeStatus(checks),
